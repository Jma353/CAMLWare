...
let add b1 b2 = 
  ...
  let added = adder b1 b2 carryout in 
  if length added > 64 then substream added 0 63 else added


let sign_extend n b with_neg =
  let rec make n lst =
    match n with 
    | 0 -> lst
    | x ->  (make (x - 1) lst) @ [with_neg] 
  in make (n - length b) b
...
let negate b = 
  ...
  let added = adder bin (one (length b)) false in 
  if negative b then sign_extend 64 added false else sign_extend 64 added true


let subtract b1 b2 =
  let carryout = negative b1 = negative b2 in
  let b1_extended = sign_extend 64 b1 (negative b1) in 
  let added = adder b1_extended (negate b2) carryout in 
  if length added > 64 then substream added 0 63 else added
...
let bitstream_of_decimal d =
  ...
  in let bin = sign_extend 64 (divide (Pervasives.abs d)) false in 
...
let shift_left b n =
  ...
  if length shifted > 64 then substream shifted 0 63 else shifted
...
let shift_right_logical b n =
  ...
  if length shifted > 64 
  then substream shifted (length shifted - 64) (length shifted) else shifted
